...
describe("upload", () => {
  ...
  it("uploads a bot manifest with custom private key when --privateKey flag is provided", async () => {
    const mockGetCredentials = jest.fn()
    const mockUploadManifest = jest.fn()
    const mockAppendToFile = jest.fn()
    ...
    mockUploadManifest.mockReturnValueOnce(mockManifestRef)
    const mockPrivateKey = "0x4567"
    const mockCliArgs = {imageRef: 'someImageRef', refOnly: true, privateKey: mockPrivateKey}
