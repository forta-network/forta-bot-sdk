...
export default function providePushToRegistry(
  ...
  return async function pushToRegistry(manifestReference: string, fromWallet: Wallet) {
    ...
    if (!agentExists) {
      ...
      await botRegistry.createAgent(fromWallet, botId, manifestReference, chainIds)
    } else {
      ...
      await botRegistry.updateAgent(fromWallet, botId, manifestReference, chainIds)
    }

    const logMessage = `successfully ${agentExists ? 'updated' : 'added'} bot id ${botId} with manifest ${manifestReference}`
